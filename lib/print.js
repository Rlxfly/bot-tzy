...
module.exports = async function (m, itsu = {user: {}}) {
  let _name = itsu.getName(m.sender)
  let sender = PhoneNumber('+' + m.sender.replace('@s.whatsapp.net', '')).getNumber('international') + (_name ? ' ~' + _name : '')
  let chat = itsu.getName(m.chat)
  // let ansi = '\x1b['
  let img
  try {
    if (global.opts['img'])
    ...
    m.msg.vcard ?
      ...
      m.msg.fileLength ?
        ...
        m.text ?
          m.text.length :
          0
    : m.text ? m.text.length : 200
  let user = global.DATABASE.data.users[m.sender]
  let me = PhoneNumber('+' + (itsu.user && itsu.user.jid).replace('@s.whatsapp.net', '')).getNumber('international')
  console.log(`
...
`.trim(),
    ...
    filesize,
    filesize === 0 ? 0 : (filesize / 999999 ** Math.floor(Math.log(filesize) / Math.log(99999))).toFixed(1),
  ...
  if (typeof m.text === 'string' && m.text) {
    ...
    if (m.mentionedJid) for (let user of m.mentionedJid) log = log.replace('@' + user.split`@`[0], chalk.blueBright('@' + itsu.getName(user)))
    console.log(m.error != null ? chalk.red(log) : m.isCommand ? chalk.yellow(log) : log)
  }
  if (m.messageStubParameters) console.log(m.messageStubParameters.map(jid => {
    let name = itsu.getName(jid)
