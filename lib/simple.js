 ...
 */
exports.smsg = async (itsu, m, hasParent) => {
  if (!m) return m
  ...
  if (m.message) {
    ...
    m.reply = async (text, chatId, options) => {
    	let pp = await itsu.getProfilePicture(m.sender).catch(_ => 'https://telegra.ph/file/a2ae6cbfa40f6eeea0cf1.jpg')
        let { data } = await itsu.getFile(pp)
        itsu.reply(chatId ? chatId : m.chat, text, m, { contextInfo: { mentionedJid: itsu.parseMention(text), externalAdReply: { title: 'ꜱɪᴍᴩʟᴇ ᴡʜᴀᴛꜱᴀᴩᴩ ʙᴏᴛ', body: '© Relldev', sourceUrl: 'https://wa.me/c/6283820073017', thumbnail: data }}, options })
    }
    /**
     * Copy this message
     */
