...
`.trim()
    ...
    * @returns 
    */
    async sendGroupV4Invite(jid, participant, inviteCode, inviteExpiration, groupName = 'unknown subject', caption = 'Rell UwU', options = {}) {
...
async sendButtonsVid(jid, text, path, buttons = [], footer = "© Rlxfly", quoted, options) {
...
async sendButtonsGif(jid, text, path, buttons = [], footer = "© Rlxfly", quoted, options) {
            let buff = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,`[1], 'base64') : /^https?:\/\//.test(path) ? await getBuffer(path) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
            let vid = await this.prepareMessage(jid, buff, MessageType.video, { mimetype: 'video/gif '})
            let btn = []
            buttons.forEach(x => {
                btn.push({buttonId: x.id, buttonText: { displayText: x.text }, type: 1})
            })
...
async sendSticker(jid, media, quoted, opts = {}) {
   let convert = require('./sticker').sticker
   let medias = await this.getFile(media)
   return await this.sendMessage(jid, await convert(medias.data, null, opts.packname || global.packname, opts.author || global.author), 'stickerMessage', { quoted: quoted, ...opts })
}
    /**
     * Reply to a message
     * @param {String} jid
    ...
    reply(jid, text, quoted, options) {
      return Buffer.isBuffer(text) ? this.sendFile(jid, text, 'file', '', quoted, false, options) : this.sendMessage(jid, text, MessageType.extendedText, { contextInfo: { mentionedJid: this.parseMention(text),  externalAdReply: {title: 'ꜱɪᴍᴩʟᴇ ᴡʜᴀᴛꜱᴀᴩᴩ ʙᴏᴛ', body: '© Relldev', sourceUrl: '', thumbnail: global.thumb3}}, sendEphemeral: true, quoted, ...options })
    }
    /**
     * Fake Replies
     * @param {String} jid
...
/**
 * Serialize Message
 * @param {WAConnection} itsu
 * @param {Object} m
 * @param {Boolean} hasParent
 */
exports.smsg = (itsu, m, hasParent) => {
  if (!m) return m
  ...
  if (m.key) {
    ...
    m.sender = m.fromMe ? itsu.user.jid : m.participant ? m.participant : m.key.participant ? m.key.participant : m.chat
  }
  if (m.message) {
    m.mtype = Object.keys(m.message)[0]
    m.msg = m.message[m.mtype]
    if (m.mtype === 'ephemeralMessage') {
      exports.smsg(itsu, m.msg)
      m.mtype = m.msg.mtype
      m.msg = m.msg.msg
    }
    ...
    if (m.quoted) {
      ...
      m.quoted.fromMe = m.quoted.sender === (itsu.user && itsu.user.jid)
      m.quoted.text = m.quoted.text || m.quoted.caption || ''
      m.quoted.mentionedJid = m.quoted.contextInfo ? m.quoted.contextInfo.mentionedJid : []
      m.getQuotedObj = m.getQuotedMessage = async () => {
        if (!m.quoted.id) return false
        let q = await itsu.loadMessage(m.chat, m.quoted.id)
        return exports.smsg(itsu, q)
      }
      ...
      if (m.quoted.url) m.quoted.download = (type = 'buffer') => itsu.downloadM(vM, type)
      /**
       ...
       * @param {Object} options
       */
      m.quoted.reply = (text, chatId, options) => itsu.reply(chatId ? chatId : m.chat, text, vM, options)
      /**
       * Copy quoted message
       */
      m.quoted.copy = () => exports.smsg(itsu, M.fromObject(M.toObject(vM)))
      /**
       * Forward quoted message
       * @param {String} jid
       * @param {Boolean} forceForward
       */
      m.quoted.forward = (jid, forceForward = false) => itsu.forwardMessage(jid, vM, forceForward)
      /**
       ...
       * @param {Object} options
       */
      m.quoted.copyNForward = (jid, forceForward = false, options = {}) => itsu.copyNForward(jid, vM, forceForward, options)
      /**
       ...
       * @param {Object} options
       */
      m.quoted.cMod = (jid, text = '', sender = m.quoted.sender, options = {}) => itsu.cMod(jid, vM, text, sender, options)
      /**
       * Delete quoted message
       */
      m.quoted.delete = () => itsu.deleteMessage(m.quoted.chat, vM.key)
    }
    if (m.msg.url) m.download = (type = 'buffer') => itsu.downloadM(m, type)
     ...
     * @param {Object} options
     */
    m.reply = (text, chatId, options) => itsu.reply(chatId ? chatId : m.chat, text, m, options)
    /**
     * Copy this message
     */
    m.copy = () => exports.smsg(itsu, M.fromObject(M.toObject(m)))
    /**
     * Forward this message
     * @param {String} jid
     * @param {Boolean} forceForward
     */
    m.forward = (jid = m.chat, forceForward = false) => itsu.forwardMessage(jid, m, forceForward)
    /**
     ...
     * @param {Object} options
     */
    m.copyNForward = (jid = m.chat, forceForward = false, options = {}) => itsu.copyNForward(jid, m, forceForward, options)
    /**
     * Modify this Message
     * @param {String} jid
     * @param {String} text
     ...
     * @param {Object} options
     */
    m.cMod = (jid, text = '', sender = m.sender, options = {}) => itsu.cMod(jid, m, text, sender, options)
  }
  return m
}
